...
export const RL318 = () => {
    ...
    const getRL = async (e) => {
        ...
        try {
            ...
            const results = await axiosJWT.get('/apisirs6v2/rltigatitikdelapanbelas',
                customConfig)
            ...
            const total_rawat_jalan = rlTigaTitikDelapanBelasDetails.reduce((previousValue, currentValue) => {
                return previousValue + currentValue.rawat_jalan;
            }, 0);

            const total_igd = rlTigaTitikDelapanBelasDetails.reduce((previousValue, currentValue) => {
                return previousValue + currentValue.igd;
            }, 0);

            const total_rawat_inap = rlTigaTitikDelapanBelasDetails.reduce((previousValue, currentValue) => {
    ...
    const deleteRL = async (id) => {
        ...
        try {
            await axiosJWT.delete(`/apisirs6v2/rltigatitikdelapanbelas/${id}`,
                customConfig)
        ...
        <div className="row">
            <div className="col-md-12">
                <div style={{marginBottom: "10px"}}>
                    {
                        user.jenisUserId === 4 ? (
                                <Link className='btn' to={`/rl318/tambah/`} style={{ marginRight: "5px", fontSize: "18px", backgroundColor: "#779D9E", color: "#FFFFFF" }}>
                                    +
                                </Link>
                        ...
                        <tbody>
                            {dataRL.map((value, index) => {
                                return (
                                    <tr key={value.id}>
                                    <td>
                                        <input type='text' name='id' className="form-control" value={value.no_golongan_obat} disabled={true} />
                                    </td>
                            ...
                            })}
                            {
                                ...
                                <tr>
                                    ...
                                    <td><center>{total_rawat_jalan}</center></td>
                                    <td><center>{total_igd}</center></td>
                                    <td><center>{total_rawat_inap}</center></td>
                                </tr>
                                 ) : (
                                    <></>
                                  )
